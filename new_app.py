...
def submit_synthesis(query,avatar,voice):
    ...
    if response.status_code < 400:
        ...
        st.info('Batch avatar synthesis job submitted successfully')
        logger.info(f'Job ID: {response.json()["id"]}')
        return response.json()["id"]
    else:
        logger.error(f'Failed to submit batch avatar synthesis job: {response.text}')
        st.error(f'Failed to submit batch avatar synthesis job: {response.text}')
  
  
if __name__ == '__main__':
    #query = "Hi i'm virtual assistant from microsoft"
